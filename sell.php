...
<nav style='width:1200px;height:68px;background-color:#342e48;margin:auto;'>
	...
	<div class='nav-box' style='border-bottom:2px solid #4fe3c1;'><a href='sell.php' style='text-decoration:none;color:#fff;;font-size:14px;'>销售</a></div>
...
<div class='main-wrapper' id='main-wrapper'>
    ...
    <div class='selling-block block'>
    	<p>销售单</p>
        <form method='post' action='php/submitSell.php'>
...
$('#supplyInfo').click(function(){
	JD=true;
		...
		success:function(raw_data){
			...
			for(var i=0;data[i]['bookId']!='';i++){
				if($('#supplyBookId').val().replace(/\b(0+)/gi,'')==data[i]['bookId'].replace(/\b(0+)/gi,'')&data[i]['stock']-$('#amount').val()>0){
					JD=true;
					break;
				}
			}
	...
	//return JD;
	if(JD){
		alert($('#supplyBookId').val()+' 号书 销售 '+$('#amount').val()+' 本');
		return true;
	}else{
		alert('库存里面没有该书');
		return false;
